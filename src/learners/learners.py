...
class BaseLearner:
    ...
    def train_reward_predictor(self, buffer):
        """
        Train the reward predictor network, with parallelization if enabled.
        """
        rp_log_dict = self.reward_predictor.train_predictor(buffer=buffer, batch_size=self.batch_size, verbose=False)
        return rp_log_dict
...
class IndependentRLRPLearner(BaseLearner):
    def learn_dqn(self,
        total_timesteps,
        callback,
        log_interval,
        ...
        while self.total_steps < total_timesteps:
            ...
            rollout = self.rl_agent.collect_rollouts(
                ...
                replay_buffers=[rl_agent.replay_buffer for rl_agent in self.rl_agent.agents],
            ...
            if self.total_episodes > self.rp_learning_starts and self.total_episodes > ep_cnt:
                for rl_agent in self.rl_agent.agents:
                    buffer = rl_agent.replay_buffer
                    rp_log_dict = self.train_reward_predictor(buffer=buffer)
...
class CollectiveRLRPLearner(BaseLearner):
    ...
    def learn_dqn(self,
        ...
        while self.total_steps < total_timesteps:
            ...
            # Train the reward predictor periodically
            if self.total_episodes > self.rp_learning_starts and self.total_episodes > ep_cnt:
                buffer = self.rl_agent.replay_buffer
                rp_log_dict = self.train_reward_predictor(buffer)
                for k, v in rp_log_dict.items():
                    self.rl_agent.logger.record(k, v)
            # Train the RL agent
            gradient_steps = self.rl_agent.gradient_steps if self.rl_agent.gradient_steps >= 0 else rollout.episode_timesteps
