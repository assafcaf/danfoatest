...
class MapEnv(gymnasium.Env):
    ...
    def __init__(self, ascii_map, num_agents=1, render=True, color_map=None, same_color=False, full_state=False, scope=False):
        """
    ...
    def step(self, actions):
        ...
        for agent in self.agents.values():
            ...
            rgb_arr = self.single_agent_map_to_colors(agent.get_state(), self.color_map, agent=agent)
    ...
    def map_to_colors(self, map=None, color_map=None):
        ...
        if self.scope:
        # Add scope (gray square) in front of each agent
            for agent in self.agents.values():
                pos = agent.get_pos()
                orientation = agent.get_orientation()  # Assuming this method exists
                scope_offset = ORIENTATIONS[orientation]  # Get movement direction
                scope_pos = pos + scope_offset

                # Ensure scope stays in bounds
                if 1 < scope_pos[0] < rgb_arr.shape[0]-1 and 1 < scope_pos[1] < rgb_arr.shape[1]-1:
                    rgb_arr[scope_pos[0], scope_pos[1], :] = self.color_map["C"]  # Gray square
        return rgb_arr

    def single_agent_map_to_colors(self, map=None, color_map=None, agent=None):
        """Converts a map to an array of RGB values.
        Parameters
        ----------
        map: np.ndarray
        ...
        # Add scope (gray square) in front of each agent
        if self.scope:
            rgb_arr = self.adjust_single_agent_obs(rgb_arr, agent.orientation, color_map)

        return rgb_arr
    
    def adjust_single_agent_obs(self, rgb_arr, orientation, color_map):
        m = rgb_arr.shape[0]//2
        scope_offset = ORIENTATIONS[orientation]  # Get movement direction
        scope_x = np.max([0, 0 + scope_offset[0]])
        sope_y = np.max([0, m + scope_offset[1]])
        # Ensure scope stays in bounds
        if rgb_arr[(scope_x, sope_y)].sum() == 0:
            rgb_arr[(scope_x, sope_y)] = color_map['C'] # Gray square
        return rgb_arr
    ...
    def update_map_fire(self, firing_pos, firing_orientation, fire_len, fire_char, cell_types=[],
        ...
        firing_pos = [
                start_pos, #midle
                start_pos + right_shift*2 - firing_direction, 
